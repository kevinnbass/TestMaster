...
class RealTimeMonitor:
    ...
    def __init__(self, mode: MonitoringMode = MonitoringMode.FULL, update_interval: float = 1.0):
        ...
        # LLM monitor available but NOT automatically started
        self.llm_monitor = None
        try:
            from llm_analysis_monitor import get_llm_monitor
            self.llm_monitor = get_llm_monitor(demo_mode=True)  # Force demo mode to prevent auto calls
            print("[PASS] LLM Analysis Monitor available (user-triggered only)")
        except ImportError:
    ...
    def _collect_component_metrics(self):
        ...
        self.current_metrics.workflow_queue_size = 0  # Would need real queue implementation
        
        # Real events based on system activity
        try:
            import psutil
            net_io = psutil.net_io_counters()
            # Network packets as proxy for events
            self.current_metrics.events_per_second = min(100.0, 
                (net_io.packets_sent + net_io.packets_recv) / 1000.0)
        except:
            self.current_metrics.events_per_second = 0.0
        
        # Real consensus decisions (would need actual consensus engine)
        self.current_metrics.consensus_decisions = 0
        
        # Real security alerts (would need actual security scanner)  
        self.current_metrics.security_alerts = 0
