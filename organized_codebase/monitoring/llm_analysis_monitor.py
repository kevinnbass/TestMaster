...
@dataclass
class LLMAPICall:
    """Track individual LLM API calls with codebase tagging."""
    timestamp: datetime
...
class LLMAnalysisMonitor:
    ...
    def __init__(self, api_key: Optional[str] = None, demo_mode: bool = False):
        ...
        if GENAI_AVAILABLE and self.api_key and not demo_mode:
            try:
                ...
                print("[PASS] Gemini SDK initialized with gemini-2.5-flash ONLY")
        ...
        # Always start in user-triggered mode - no automatic API calls
        print("[MODE] User-triggered API calls only - no automatic monitoring")
        self.demo_mode = True  # Force demo mode to prevent automatic calls
        self.user_triggered_mode = True
        # NEVER setup demo data - all data must be real
    ...
    def start_monitoring(self):
        """Enable monitoring (but don't start automatic analysis)."""
        print("[NOTICE] LLM Analysis Monitor enabled for user-triggered requests only")
        print("[NOTICE] No automatic API calls will be made")
        self.monitoring_active = True
        # Do NOT start monitoring thread - only respond to user requests
    
    def analyze_module_on_demand(self, module_path: str, codebase: str = None) -> Optional[ModuleAnalysis]:
        """Analyze a module when user explicitly requests it (button click)."""
        if not self.gemini_available:
            print(f"[ERROR] Gemini 2.5 Flash not available for analysis of {module_path}")
            return None
        
        print(f"[USER REQUEST] Analyzing {module_path} for codebase {codebase} with Gemini 2.5 Flash...")
        # Pass codebase to the analysis method
        return self.analyze_module_with_gemini(module_path, codebase)
    ...
    def track_api_call(self, provider: LLMProvider, model: str, analysis_type: AnalysisType,
                      ...
                      module_path: Optional[str] = None, codebase: Optional[str] = None) -> None:
        """Track an LLM API call with codebase tagging."""
        ...
        print(f"API call tracked: {provider.value} - {analysis_type.value} - "
              f"{total_tokens} tokens - ${cost_estimate:.4f} - codebase: {codebase}")
    
    def get_codebase_metrics(self, codebase: str = None) -> Dict[str, Any]:
        """Get metrics for a specific codebase or all if None."""
        if codebase:
            # Filter calls for specific codebase
            codebase_calls = [call for call in self.api_calls if call.codebase == codebase]
        else:
            codebase_calls = self.api_calls
        
        recent_calls = [call for call in codebase_calls 
                       if (datetime.now() - call.timestamp).total_seconds() < 3600]  # Last hour
        
        total_calls = len(codebase_calls)
        successful_calls = len([c for c in codebase_calls if c.success])
        failed_calls = len([c for c in codebase_calls if not c.success])
        total_input_tokens = sum(c.input_tokens for c in codebase_calls)
        total_output_tokens = sum(c.output_tokens for c in codebase_calls)
        total_cost = sum(c.cost_estimate for c in codebase_calls)
        
        calls_per_minute = 0
        avg_response_time = 0
        if recent_calls:
            calls_per_minute = len([c for c in recent_calls 
                                  if (datetime.now() - c.timestamp).total_seconds() < 300]) / 5  # last 5 mins
            avg_response_time = sum(c.duration_ms for c in recent_calls) / len(recent_calls)
        
        return {
            'codebase': codebase or 'all',
            'api_calls': {
            ...
            'analysis_status': {
                'active_analyses': len([path for path, timestamp in self.active_analyses.items() 
                                      if (datetime.now() - timestamp).total_seconds() < 300]),
                'completed_analyses': len([a for a in self.analysis_results.values() 
                                         if codebase is None or a.module_path.startswith(codebase or '')])
            },
            'recent_activity': [
                {
                    'timestamp': call.timestamp.isoformat(),
                    'module': call.module_path,
                    'type': call.analysis_type.value,
                    'tokens': call.total_tokens,
                    'cost': call.cost_estimate,
                    'success': call.success
                } for call in recent_calls[-5:]  # Last 5 calls
            ]
        }
    
    def analyze_module_with_gemini(self, module_path: str, codebase: str = None) -> Optional[ModuleAnalysis]:
        ...
        try:
            ...
            self.track_api_call(
                provider=LLMProvider.GEMINI,
                model=self.model_name if hasattr(self, 'model_name') else 'gemini-2.5-flash',
        ...
        def demo_analyze_worker():
            try:
                ...
                self.track_api_call(
                    ...
                    success=True,
                    module_path=module_path,
                    codebase=codebase if 'codebase' in locals() else None
                )
