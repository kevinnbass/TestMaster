    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        with self.lock:
            ...
            if entry.compressed and isinstance(value, bytes):
                value = SafePickleHandler.safe_load(zlib.decompress(value))
...
def get_cache() -> IntelligentCache:
    ...
    if _global_cache is None:
        # Use TestMaster configuration (compatible with enhanced unified config)
        from config.testmaster_config import config
        
        # Alternative: Use enhanced unified configuration 
        # from config import get_caching_config
        # config_obj = get_caching_config()
