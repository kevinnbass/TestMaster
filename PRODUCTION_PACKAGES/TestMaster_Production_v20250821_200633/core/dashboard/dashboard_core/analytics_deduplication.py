"""
Analytics Duplication Detection and Deduplication System
======================================================
...
class AnalyticsDeduplication:
    ...
    def __init__(self,
        ...
        self.content_index: Dict[str, Set[str]] = defaultdict(set)  # hash -> analytics_ids
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute('''
                    CREATE TABLE IF NOT EXISTS merged_analytics (
                        merge_id TEXT PRIMARY KEY,
                        source_analytics_ids TEXT NOT NULL,
                        merged_analytics_id TEXT NOT NULL,
                ...
                conn.execute('CREATE INDEX IF NOT EXISTS idx_semantic_hash ON analytics_fingerprints(semantic_hash)')
                conn.execute('CREATE INDEX IF NOT EXISTS idx_temporal_hash ON analytics_fingerprints(temporal_hash)')
                conn.execute('CREATE INDEX IF NOT EXISTS idx_duplicate_type ON duplicate_records(duplicate_type)')
                conn.execute('CREATE INDEX IF NOT EXISTS idx_detected_at ON duplicate_records(detected_at)')
        ...
        with self.lock:
            try:
                ...
                if duplicates:
                    ...
                    # Return False to indicate this might be a duplicate
                    return False
    ...
    def _extract_semantic_elements(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for field in priority_fields:
                if field in data:
                    semantic[field] = str(data[field])[:100]  # Limit length
            
            # Extract error/exception information
            for key, value in data.items():
                if any(keyword in key.lower() for keyword in ['error', 'exception', 'fail', 'issue']):
            ...
            if numerical_fields:
                semantic['numerical_summary'] = numerical_fields
        ...
        try:
            ...
            for field in timestamp_fields:
                if field in data:
                    ...
                    try:
                        # Parse timestamp and round to temporal window
    ...
    def _detect_duplicates(self, fingerprint: AnalyticsFingerprint) -> Dict[DuplicateType, List[Tuple[str, float]]]:
        ...
        try:
            ...
            # Content duplicate check (semantic similarity)
            semantic_matches = self.semantic_index.get(fingerprint.semantic_hash, set())
            if semantic_matches:
                content_duplicates = []
                for match_id in semantic_matches:
                    if match_id in self.fingerprints:
                        match_fp = self.fingerprints[match_id]
                        similarity = self._calculate_content_similarity(fingerprint, match_fp)
                        if similarity >= self.similarity_threshold:
                            content_duplicates.append((match_id, similarity))
                
                if content_duplicates:
                    duplicates[DuplicateType.CONTENT] = content_duplicates
            
            # Temporal duplicate check (same time window)
            temporal_matches = self.temporal_index.get(fingerprint.temporal_hash, set())
            if temporal_matches:
                temporal_duplicates = []
                for match_id in temporal_matches:
                    if match_id in self.fingerprints:
                        match_fp = self.fingerprints[match_id]
                        similarity = self._calculate_temporal_similarity(fingerprint, match_fp)
                        if similarity >= self.manual_review_threshold:  # Lower threshold for temporal
                            temporal_duplicates.append((match_id, similarity))
                
                if temporal_duplicates:
                    duplicates[DuplicateType.TEMPORAL] = temporal_duplicates
    ...
    def _calculate_semantic_similarity(self, fp1: AnalyticsFingerprint, fp2: AnalyticsFingerprint) -> float:
        ...
        try:
            ...
            # Compare sizes (similar sizes suggest similar content)
    ...
    def _calculate_content_similarity(self, fp1: AnalyticsFingerprint, fp2: AnalyticsFingerprint) -> float:
        """Calculate content similarity between fingerprints."""
        try:
            # Use difflib for text similarity
            keys1 = set(fp1.key_fields.keys())
            keys2 = set(fp2.key_fields.keys())
            
            # Structure similarity
            structure_similarity = len(keys1 & keys2) / len(keys1 | keys2) if keys1 | keys2 else 1.0
            ...
            if common_keys:
                ...
                for key in common_keys:
                    val1 = fp1.key_fields[key]
                    val2 = fp2.key_fields[key]
                    
                    if val1 == val2:
                        similarities.append(1.0)
                    else:
                        # Use sequence matching for string similarity
            ...
            return (structure_similarity * 0.4) + (value_similarity * 0.6)
            
        except Exception as e:
            logger.error(f"Content similarity calculation failed: {e}")
            return 0.0
    
    def _calculate_temporal_similarity(self, fp1: AnalyticsFingerprint, fp2: AnalyticsFingerprint) -> float:
        """Calculate temporal similarity between fingerprints."""
        try:
            # Time difference similarity
            time_diff = abs((fp1.timestamp - fp2.timestamp).total_seconds())
            max_window = self.temporal_window_minutes * 60
            
            if time_diff <= max_window:
                time_similarity = 1.0 - (time_diff / max_window)
            else:
                time_similarity = 0.0
            
            # Also consider content similarity for temporal matches
            content_sim = self._calculate_content_similarity(fp1, fp2)
            
            # Weighted combination
            return (time_similarity * 0.6) + (content_sim * 0.4)
            ...
            try:
                if self.processing_queue:
                    with self.lock:
                        ...
                        # Process each duplicate in batch
    ...
    def _handle_exact_duplicate(self, record: DuplicateRecord, action: DeduplicationAction) -> bool:
        ...
        try:
            if action == DeduplicationAction.DISCARD:
                # Simply mark as discarded - the duplicate won't be processed further
                logger.info(f"Discarded exact duplicate: {record.duplicate_analytics_id}")
                return True
            elif action == DeduplicationAction.MERGE:
                # For exact duplicates, just keep one
                logger.info(f"Kept original for exact duplicate: {record.original_analytics_id}")
                return True
    ...
    def _handle_near_duplicate(self, record: DuplicateRecord, action: DeduplicationAction) -> bool:
        ...
        try:
            if action == DeduplicationAction.MERGE:
                # Merge near duplicates by combining data
                merged_data = self._merge_analytics_data(
                ...
                if merged_data:
                    ...
                    self._save_merged_analytics(merge_id, [record.original_analytics_id, record.duplicate_analytics_id], merged_data)
                    logger.info(f"Merged near duplicates: {record.original_analytics_id} + {record.duplicate_analytics_id}")
                    return True
            
            elif action == DeduplicationAction.MANUAL_REVIEW:
                # Mark for manual review
                record.status = DuplicateStatus.REVIEWED
                logger.info(f"Marked for manual review: {record.duplicate_id}")
                return True
    ...
    def _handle_content_duplicate(self, record: DuplicateRecord, action: DeduplicationAction) -> bool:
        ...
        try:
            if action == DeduplicationAction.MERGE:
                # Intelligent merge based on content
                ...
                if orig_fp and dup_fp:
                    if dup_fp.timestamp > orig_fp.timestamp:
                        logger.info(f"Keeping newer analytics: {record.duplicate_analytics_id}")
                    else:
                        logger.info(f"Keeping original analytics: {record.original_analytics_id}")
                    return True
    ...
    def _handle_semantic_duplicate(self, record: DuplicateRecord, action: DeduplicationAction) -> bool:
        ...
        try:
            # Semantic duplicates need careful handling
            if action == DeduplicationAction.MANUAL_REVIEW:
                record.status = DuplicateStatus.REVIEWED
                logger.info(f"Semantic duplicate marked for review: {record.duplicate_id}")
                return True
    ...
    def _handle_temporal_duplicate(self, record: DuplicateRecord, action: DeduplicationAction) -> bool:
        ...
        try:
            if action == DeduplicationAction.MERGE:
                # Temporal duplicates might represent the same event
                merged_data = self._temporal_merge(
                    record.original_analytics_id,
                    record.duplicate_analytics_id
                )
    ...
    def _merge_analytics_data(self, original_id: str, duplicate_id: str) -> Optional[Dict[str, Any]]:
        """Merge analytics data from two sources."""
        try:
            # Get fingerprints
            orig_fp = self.fingerprints.get(original_id)
            dup_fp = self.fingerprints.get(duplicate_id)
            
            if not orig_fp or not dup_fp:
                return None
            
            # Simple merge strategy - combine key fields
            merged_data = {}
            
            # Merge key fields
            for key, value in orig_fp.key_fields.items():
                merged_data[key] = value
            
            # Add additional fields from duplicate
            for key, value in dup_fp.key_fields.items():
                if key not in merged_data:
                    merged_data[key] = value
                elif merged_data[key] != value:
                    # Create combined field for conflicting values
                    merged_data[f"{key}_combined"] = [merged_data[key], value]
            
            # Add metadata about merge
            merged_data['_merge_info'] = {
                'source_analytics': [original_id, duplicate_id],
            ...
            logger.error(f"Analytics data merge failed: {e}")
            return None
    
    def _intelligent_merge(self, original_id: str, duplicate_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            priority_fields = ['test_id', 'name', 'status', 'result']
            
            # Use original for priority fields
            for field in priority_fields:
                if field in orig_fp.key_fields:
                    merged_data[field] = orig_fp.key_fields[field]
            
            # Intelligent merge for other fields
            all_fields = set(orig_fp.key_fields.keys()) | set(dup_fp.key_fields.keys())
            
            for field in all_fields:
                if field in priority_fields:
                    continue  # Already handled
                
                orig_val = orig_fp.key_fields.get(field)
                dup_val = dup_fp.key_fields.get(field)
                
                if orig_val is None:
                    merged_data[field] = dup_val
                elif dup_val is None:
                    merged_data[field] = orig_val
                elif orig_val == dup_val:
                    merged_data[field] = orig_val
                else:
                    ...
                    if 'time' in field.lower() or 'date' in field.lower():
                        ...
                        try:
                            orig_time = datetime.fromisoformat(str(orig_val))
                            dup_time = datetime.fromisoformat(str(dup_val))
                            merged_data[field] = max(orig_time, dup_time).isoformat()
                        except:
                            merged_data[field] = orig_val
                    elif 'count' in field.lower() or 'num' in field.lower():
                        # Sum numerical values
                        try:
                            merged_data[field] = float(orig_val) + float(dup_val)
                        except:
                            merged_data[field] = orig_val
                    else:
                        # Keep both values
                        merged_data[f"{field}_original"] = orig_val
                        merged_data[f"{field}_duplicate"] = dup_val
            
            # Add merge metadata
            merged_data['_merge_info'] = {
                'source_analytics': [original_id, duplicate_id],
                'merged_at': datetime.now().isoformat(),
                'merge_type': 'intelligent',
                'similarity_score': self._calculate_content_similarity(orig_fp, dup_fp)
            }
    ...
    def _temporal_merge(self, original_id: str, duplicate_id: str) -> Optional[Dict[str, Any]]:
        """Temporal merge for time-based duplicates."""
        try:
            # Get fingerprints
            orig_fp = self.fingerprints.get(original_id)
            dup_fp = self.fingerprints.get(duplicate_id)
            ...
            # Use the earlier timestamp as the primary
            if orig_fp.timestamp <= dup_fp.timestamp:
                primary_fp = orig_fp
                secondary_fp = dup_fp
            else:
                primary_fp = dup_fp
                secondary_fp = orig_fp
            
            merged_data = dict(primary_fp.key_fields)
            
            # Add time range information
            time_diff = abs((orig_fp.timestamp - dup_fp.timestamp).total_seconds())
            merged_data['_temporal_info'] = {
                'time_range_seconds': time_diff,
                'earliest_timestamp': primary_fp.timestamp.isoformat(),
                'latest_timestamp': secondary_fp.timestamp.isoformat(),
                'temporal_merge': True
            }
            
            # Merge conflicting fields with temporal context
            for field, value in secondary_fp.key_fields.items():
                if field not in merged_data:
                    merged_data[field] = value
                elif merged_data[field] != value:
                    merged_data[f"{field}_temporal_sequence"] = [
                        {'value': merged_data[field], 'timestamp': primary_fp.timestamp.isoformat()},
                        {'value': value, 'timestamp': secondary_fp.timestamp.isoformat()}
                    ]
            
            return merged_data
            
        except Exception as e:
            logger.error(f"Temporal merge failed: {e}")
            return None
    ...
    def _save_merged_analytics(self, merge_id: str, source_ids: List[str], merged_data: Dict[str, Any]):
        """Save merged analytics to database."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    INSERT INTO merged_analytics
                    (merge_id, source_analytics_ids, merged_analytics_id, merge_strategy,
                     merged_at, original_data, merged_data)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    merge_id,
                    json.dumps(source_ids),
                    merge_id,  # Use merge_id as the new analytics ID
                    'intelligent',
            ...
            logger.error(f"Failed to save merged analytics: {e}")
    ...
    def _cleanup_old_fingerprints(self):
        ...
        try:
            ...
            logger.info(f"Cleaned up {removal_count} old fingerprints")
    ...
    def _cleanup_old_duplicates(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if deleted_count > 0:
                    logger.info(f"Cleaned up {deleted_count} old duplicate records")
        ...
        with self.lock:
            ...
            return {
                ...
                'index_sizes': {
                    ...
                    'structure_index': len(self.structure_index),
    ...
    def force_deduplication(self, analytics_id: str) -> bool:
        ...
        try:
            if analytics_id in self.fingerprints:
                ...
                if duplicates:
                    logger.info(f"Forced deduplication found duplicates for: {analytics_id}")
                    return True
                else:
                    logger.info(f"No duplicates found for: {analytics_id}")
                    return False
    ...
    def get_duplicate_details(self, duplicate_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    'SELECT * FROM duplicate_records WHERE duplicate_id = ?',
                    (duplicate_id,)
                )
                row = cursor.fetchone()
                ...
                if row:
                    return {
                        'duplicate_id': row[0],
                        'original_analytics_id': row[1],
                        'duplicate_analytics_id': row[2],
                        'duplicate_type': row[3],
                        'similarity_score': row[4],
                        'detected_at': row[5],
                        'action_taken': row[6],
                        'status': row[7],
                        'merge_result_id': row[8],
                        'metadata': json.loads(row[9]) if row[9] else {}
                    }
        except Exception as e:
            logger.error(f"Duplicate details lookup failed: {e}")
