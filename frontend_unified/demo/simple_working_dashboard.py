...
# Import the template and basic analysis
    ...
    try:
        ...
        if graph_file.exists():
            with open(graph_file, 'r') as f:
                ...
                return jsonify({
                    "nodes": data.get("nodes", [])[:100],  # Limit for performance
                    "relationships": data.get("relationships", [])[:200],
...
def enhanced_linkage_data():
    """Serve a simple enhanced data structure for testing."""
    try:
        # Get basic linkage data
        ...
        for category in ['orphaned_files', 'hanging_files', 'marginal_files', 'well_connected_files']:
            if category in basic_data:
                all_files.extend(basic_data[category][:10])  # Limit for performance
...
def main():
    """Launch the simple working dashboard."""
    print("STARTING Simple Working Dashboard")
    print("=" * 50)
    
    print("\nDashboard Access:")
    print("   - Main Dashboard: http://localhost:5003")
    print("   - Linkage Data API: http://localhost:5003/linkage-data")
    print("   - Graph Data API: http://localhost:5003/graph-data")
    print("   - Enhanced Data API: http://localhost:5003/enhanced-linkage-data")
    
    print("\nHow to Use:")
    print("   1. Open http://localhost:5003 in your browser")
    print("   2. Click 'Graph View' tab")
    print("   3. Click 'Load Graph' button")
    print("   4. Use the layer dropdown to switch dimensions!")
