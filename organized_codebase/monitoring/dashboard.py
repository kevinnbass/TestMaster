        ...
        return """
            ...
            function handleMessage(message) {
                switch (message.type) {
                    ...
                    case 'alert':
                        ...
                        break;
                    case 'alerts_cleared':
                        document.getElementById('alerts-list').textContent = 'No alerts';
                        break;
                }
            }
            ...
            function updateSystemStatus(status) {
                ...
                if (!status || Object.keys(status).length === 0) {
                    container.textContent = 'No status data available';
                    return;
                }
            ...
            function updateTestResults(results) {
                ...
                if (!results || results.length === 0) {
                    container.textContent = 'No recent test results';
                    return;
                }
            ...
            function updateCoverageData(coverage) {
                ...
                if (!coverage || Object.keys(coverage).length === 0) {
                    container.textContent = 'No coverage data available';
                    return;
                }
                ...
                container.textContent = html || 'Coverage data available';
            }
            
            function updateIdleModules(modules) {
                const container = document.getElementById('idle-modules');
                if (!modules || modules.length === 0) {
                    container.textContent = 'No idle modules detected';
                    return;
                }
            ...
            function updateBreakingTests(tests) {
                ...
                if (!tests || tests.length === 0) {
                    container.textContent = '✅ No breaking tests';
                    return;
                }
