...
class MultiObjectiveOptimizationAgent:
    ...
    def optimize(self, initial_solutions: List[Dict[str, Any]] = None, context: Dict[str, Any] = None):
        """Optimize solutions (delegates to internal optimizer)."""
        if initial_solutions is None:
            initial_solutions = [{'test_plan': {}, 'solution_id': 'default'}]
        if context is None:
            context = {}
        return self.optimizer.optimize(initial_solutions, context)
