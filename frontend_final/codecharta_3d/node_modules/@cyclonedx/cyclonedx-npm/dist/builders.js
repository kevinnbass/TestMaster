...
const cdx_1 = require("./cdx");
const npmRunner_1 = require("./npmRunner");
class BomBuilder {
    constructor(componentBuilder, treeBuilder, purlFactory, options, console_) {
    ...
    fetchNpmLs(projectDir, process_) {
        ...
        if ((0, _helpers_1.versionCompare)(npmVersionT, [8, 7]) >= 0) {
    ...
    makeComponent(data, type) {
        ...
        if ((0, _helpers_1.isString)(data.path)) {
            component.properties.add(new cyclonedx_library_1.Models.Property(cdx_1.PropertyNames.PackageInstallPath, data.path));
        }
        if (isDev || isDevOptional) {
            component.properties.add(new cyclonedx_library_1.Models.Property(cdx_1.PropertyNames.PackageDevelopment, cdx_1.PropertyValueBool.True));
        }
        if (data.extraneous === true) {
            component.properties.add(new cyclonedx_library_1.Models.Property(cdx_1.PropertyNames.PackageExtraneous, cdx_1.PropertyValueBool.True));
        }
        if (data.private === true || _dataC.private === true) {
            component.properties.add(new cyclonedx_library_1.Models.Property(cdx_1.PropertyNames.PackagePrivate, cdx_1.PropertyValueBool.True));
        }
        if ((data.inBundle ?? data._inBundle) === true) {
            component.properties.add(new cyclonedx_library_1.Models.Property(cdx_1.PropertyNames.PackageBundled, cdx_1.PropertyValueBool.True));
        }
    ...
    finalizePathProperties(rootPath, components) {
        ...
        for (const component of components) {
            for (const property of component.properties) {
                if (property.name !== cdx_1.PropertyNames.PackageInstallPath) {
                    continue;
                }
                if (property.value === '') {
    ...
    *makeToolCs() {
        const packageJsonPaths = [
            [path.resolve(module.path, '..', 'package.json'), cyclonedx_library_1.Enums.ComponentType.Application]
        ];
        const libs = [
