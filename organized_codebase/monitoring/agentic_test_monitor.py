...
class ModuleChange:
    ...
    Tracks various types of module changes including creation, modification,
    deletion, renaming, splitting, and merging. Used by the monitoring system
    to understand code evolution and maintain test synchronization.
    
    Attributes:
        path: Current path to the module
        change_type: Type of change ('created', 'modified', 'deleted', 
                     'renamed', 'split', 'merged')
        old_path: Previous path for renamed modules (None for other changes)
        related_modules: List of related module paths for splits/merges
        timestamp: ISO format timestamp of when change was detected
        
    Example:
        >>> change = ModuleChange(
        ...     path="src/new_calculator.py",
        ...     change_type="renamed",
        ...     old_path="src/calculator.py"
        ... )
        >>> print(f"Module renamed from {change.old_path} to {change.path}")
        Module renamed from src/calculator.py to src/new_calculator.py
    """
    path: str
    ...
    def __post_init__(self):
        ...
        Sets timestamp to current time if not provided and initializes
        empty list for related_modules if not specified.
        """
...
class RefactoringTracker:
    ...
    Monitors code refactoring activities including renames, splits, and merges.
    Uses content hashing and similarity analysis to detect refactoring patterns
    and maintain a history of module evolution for test synchronization.
    
    Key Capabilities:
    - Detects module renames by tracking content hashes
    - Identifies module splits (one module → multiple modules)
    - Recognizes module merges (multiple modules → one module)
    - Maintains persistent history of refactoring changes
    - Calculates content similarity for refactoring detection
    
    Attributes:
        tracking_file: Path to JSON file storing refactoring history
        history: Dictionary containing changes and module mappings
        module_hashes: Cache of module content hashes for comparison
        
    Example:
        >>> tracker = RefactoringTracker()
        >>> changes = tracker.detect_refactoring(old_modules, new_modules)
        >>> for change in changes:
        ...     print(f"{change.change_type}: {change.path}")
        renamed: src/new_calculator.py
    """
    
    def __init__(self, tracking_file: str = "refactoring_history.json"):
        """Initialize refactoring tracker with history file.
        
        Args:
            tracking_file: Path to JSON file for persistent history storage
                          (default: "refactoring_history.json")
        """
    ...
    def get_module_hash(self, file_path: Path) -> str:
        ...
        Creates a hash based on function and class signatures rather than
        full content, allowing detection of moves/renames even with minor
        changes like whitespace or comments.
        
        Args:
            file_path: Path to the Python module to hash
            
        Returns:
            MD5 hash string of sorted function/class signatures,
            or None if file cannot be parsed
            
        Algorithm:
            1. Parse module AST to extract signatures
            2. Sort signatures for consistent hashing
            3. Generate MD5 hash of sorted signatures
            
        Example:
            >>> tracker = RefactoringTracker()
            >>> hash1 = tracker.get_module_hash(Path("old/calculator.py"))
            >>> hash2 = tracker.get_module_hash(Path("new/calc.py"))
            >>> if hash1 == hash2:
            ...     print("Same module, just moved/renamed")
        """
        try:
...
class AgenticTestMonitor:
    ...
    Orchestrates automatic test maintenance by monitoring code changes,
    detecting refactorings, and updating tests accordingly. Supports three
    operational modes: single check, continuous monitoring, and idle-triggered.
    
    Key Capabilities:
    - Detects module renames and updates test imports automatically
    - Generates tests for newly created modules
    - Handles module splits by creating separate tests
    - Manages module merges by consolidating tests
    - Integrates with git for change tracking
    - Supports idle detection for non-intrusive operation
    
    Attributes:
        check_interval: Minutes between checks in continuous mode
        after_idle: Idle minutes required before after-idle mode triggers
        tracker: RefactoringTracker for change detection
        maintainer: TestMaintainer for test updates
        last_check: Timestamp of most recent check
        module_snapshot: Current set of module paths
    ...
    Example:
        ...
        >>> monitor = AgenticTestMonitor(check_interval_minutes=120)
        >>> monitor.run_continuous()  # Runs until Ctrl+C
        
        >>> # Run after 10 minutes of idle
        >>> monitor = AgenticTestMonitor(after_idle_minutes=10)
        >>> monitor.run_after_idle()
    """
